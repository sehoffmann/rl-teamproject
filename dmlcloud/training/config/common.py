...
class ConfigVar:
    def _default_parse_fn(self, config, args):
        if getattr(args, self.name, None) is not None:
            setattr(config, self.name, getattr(args, self.name))
...
class ArgparseVar(ConfigVar):
    ...
    def __init__(self, *args, option=None, add_argument_fn=_default_add_fn, parse_argument_fn=ConfigVar._default_parse_fn, **kwargs):
...
class SubConfig:
    def __init__(self, root_dct, key=None):
