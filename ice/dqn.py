...
class DQNAgent:
    ...
    def train(self, num_frames: int) -> (List[int], List[int]):
        ...
        for frame_idx in range(1, num_frames + 1):
            ...
            if frame_idx % self.frame_interval == 0:
                ...
                print(self.epsilon)
                print(f'Frame: {frame_idx} | # Games: {num_games} | WR: {100*wins/len(scores):.2f}% | Avg Score: {frame_scores[-1]} | Avg. Loss: {np.mean(losses[-self.frame_interval:])}')
                if self.plot:
    ...
    def set_epsilon(self):
        ...
        decay_factor = 1 / self.epsilon_decay
        decayed = self.epsilon - (self.max_epsilon - self.min_epsilon) * decay_factor
        self.epsilon = max(self.min_epsilon, decayed)
